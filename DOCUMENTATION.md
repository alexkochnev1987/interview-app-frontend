...
| **Sonner toast** | Transient outcome of a **user action** right after the API responds | See **Toast sources** below — never fire from `useEffect` on `error` or mutation state |
| **EmptyState / inline copy + Retry** | Passive load failures (no user click yet) | Question feed, facets, similarity search, page gates (`FlashErrorPageFallback`), take invalid link — derive copy from `query.error` via `getErrorMessage` in `src/lib/api-error.ts`; show on the page; do not pair with a duplicate toast |

Do not use `useEffect` + module-level dedupe to fire toasts when `error` or `result` state changes.

**Toast sources (user actions):**

- **Question create / save / delete / restore** — TanStack Query mutations in `src/components/questions/use-question-mutations.ts`. Success and error toasts run in hook `onSuccess` / `onError` (copy from `useToastMessages()`). Delete errors use `getDeleteQuestionErrorTitle` in `src/lib/api-error.ts` for “question in use” (409). UI components call `mutateAsync` (or `mutate`) only; they may use `try/catch` to skip navigation or form updates on failure, but must not call `runMutation` or duplicate toasts for these actions.
- **Question bulk delete** — `useBulkDeleteQuestions` invalidates the library only (no default toasts). `questions-library-client.tsx` calls `notifyBulkDeleteOutcome` on success and `notifyError` with `getErrorMessage` in `catch` on hard failure.
