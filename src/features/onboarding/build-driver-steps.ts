...
type BuildDriverStepsParams = {
  ...
  prepareStep: (step: ResolvedOnboardingStep, detail?: OnboardingEventDetail) => Promise<boolean>
  onStepActive: (step: ResolvedOnboardingStep) => void
  onDone: () => void
}

async function waitForDriverSettle(driver: Driver, expectedIndex: number): Promise<void> {
  const maxFrames = 30
  let stableFrames = 0
...
function navigateToStepIndex(driver: Driver, fromIndex: number, targetIndex: number) {
...
function lockPopoverToTarget(
  ...
  window.requestAnimationFrame(() => {
    ...
    const preferredLeft = align === 'end' ? targetRect.right - wrapperRect.width : targetRect.left
    const left = Math.min(
    ...
    if (arrow) {
      ...
      arrow.classList.add(
        side === 'bottom' ? 'driver-popover-arrow-side-bottom' : 'driver-popover-arrow-side-top',
        align === 'end' ? 'driver-popover-arrow-align-end' : 'driver-popover-arrow-align-start',
      )
    }
  })
}

function enforcePopoverGap(
  element: Element | undefined,
  driver: Driver,
  ...
  const popover = driver.getState('popover') as { wrapper?: HTMLElement } | undefined
  const wrapper = popover?.wrapper
  if (!wrapper) return
...
function applyStageRadius(driver: Driver, radius?: number) {
...
function setNextDisabled(driver: Driver, disabled: boolean) {
  const apply = () => {
    const popover = driver.getState('popover') as
      | {
...
function focusTourInput(input: HTMLInputElement | HTMLTextAreaElement) {
  window.requestAnimationFrame(() => {
    window.requestAnimationFrame(() => {
...
function setNavigationDisabled(driver: Driver, disabled: boolean) {
  const popover = driver.getState('popover') as
    | {
...
export function buildDriverSteps({
  ...
  const moveFromActiveStep = async (
    ...
    try {
      ...
      await moveToPreparedStep(driver, fromIndex, targetIndex, direction, token, detail)
    } finally {
  ...
  return steps.map((step, index) => {
    ...
    const lastEventWithNext =
      isEventGatedStep && isLast && step.advance?.allowNextOnLastEventStep === true
    ...
    return {
      ...
      popover: {
        ...
        showButtons: isEventGatedStep
          ? lastEventWithNext
            ...
            : previousStep
              ...
              : ['close']
          : ['next', 'previous', 'close'],
        nextBtnText: lastEventWithNext ? labels.next : isLast ? labels.done : labels.next,
        prevBtnText: labels.back,
        doneBtnText: labels.done,
        onPopoverRender: hasNonEmptyInputGate
          ? (popover, { driver }) => {
              const activeElement = driver.getState('activeElement') as Element | undefined
        ...
        onNextClick: async (element, _driveStep, { driver }) => {
          if (step.gate?.type === 'nonEmptyInput' && !hasNonEmptyInputValue(element)) {
            return
          }
      ...
      onHighlighted: (element, _driveStep, { driver }) => {
        ...
        cleanupEventListener = listenForOnboardingEvent(step.advance.eventName, async (detail) => {
          ...
          try {
            ...
            await moveToPreparedStep(driver, fromIndex, fromIndex + 1, 1, token, detail)
          } finally {
            finishNavigation(driver, token)
          }
        })
