...
import { assignableRoleRadioOptionsForActor, type TeamMemberRole } from '../team-roles'
...
export function useTeamChangeRole(
  ...
  const [selectedRole, setSelectedRole] = useState<TeamMemberRole>(member.role as TeamMemberRole)
  ...
  async function handleApply() {
    ...
    setLoading(true)
    try {
      const updated = await runMutation(() => updateUserRole(member.id, selectedRole), {
        successMessage: toastMessages.team.updateSuccess,
        errorMessage: toastMessages.team.updateError,
        getSuccessDescription: (data) =>
          toastMessages.team.updateSuccessDescription(data.name, sharedLabels.role(data.role)),
      })
