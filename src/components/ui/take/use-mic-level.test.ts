...
describe('useMicLevel', () => {
  it('initializes with level 0 and sets up the effect', () => {
    ...
    vi.mocked(useState).mockReturnValue([0, setLevel])
    ...
    expect(useEffect).toHaveBeenCalledWith(expect.any(Function), [null])
  })
  ...
  it('can be called with a mocked stream', () => {
    ...
    expect(level).toBe(0)
    expect(useEffect).toHaveBeenCalledWith(expect.any(Function), [mockStream])
  })
})
