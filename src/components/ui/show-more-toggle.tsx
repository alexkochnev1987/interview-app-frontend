...
export function ShowMoreToggle({ className, expanded, children, ...props }: ShowMoreToggleProps) {
  return (
    <button
      type="button"
      className={cn(
      ...
      {...props}
    >
      {expanded ? <ChevronRight className="size-3" /> : <ChevronDown className="size-3" />}
      {children}
    </button>
  )
}
