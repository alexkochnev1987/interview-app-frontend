...
describe('interviews-query-state', () => {
  it('truncates q from search params to the server limit', () => {
    ...
    expect(readInterviewsFromSearchParams(params).q).toHaveLength(MAX_INTERVIEWS_Q_LENGTH)
  })

  it('clamps typed search input to the server limit', () => {
    expect(clampInterviewsSearchQuery('x'.repeat(150))).toHaveLength(MAX_INTERVIEWS_Q_LENGTH)
