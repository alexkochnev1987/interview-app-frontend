...
type LocalizedQuestionTranslations = Schemas['CreateQuestionDto']['translations'] &
...
export type InterviewBehaviorRisk = NonNullable<
  Schemas['InterviewResultResponseDto']['behaviorSummary']
>['riskLevel']
...
export type PublicCandidateFeedbackResponse = Schemas['PublicCandidateFeedbackResponseDto']
export type PublicCandidateFeedbackTextBlock = Schemas['PublicCandidateFeedbackTextBlockDto']
...
async function postWithQuery<T>(path: string, query?: Record<string, string>): Promise<T> {
...
function normalizeTranslations(
  ...
  | Schemas['UpdateQuestionDto']['translations']
  | undefined {
...
export async function fetchHrUsers(init?: { signal?: AbortSignal }): Promise<AssignedHr[]> {
  const pageSize = 200
...
export async function getAuthMe(): Promise<MeResponse> {
  return handle(client.GET('/auth/me', LOCALIZED_HEADERS)) as Promise<MeResponse>
}
...
export type QuestionDeleteBlockingInterview = Schemas['QuestionDeleteBlockingInterviewDto']
...
export async function restoreQuestion(id: string): Promise<Question> {
  return handle(
    client.PATCH('/questions/{id}/restore', {
      ...LOCALIZED_HEADERS,
      params: { path: { id } },
    }),
  )
...
export async function deleteQuestionsBulk(ids: string[]): Promise<BulkDeleteResult> {
  return handle(
    client.POST('/questions/bulk-delete', {
      ...LOCALIZED_HEADERS,
      body: { ids },
    }),
  )
...
export async function cancelInterview(id: string): Promise<InterviewCancelResponse> {
  return handle(
    client.PATCH('/interviews/{id}/cancel', {
      params: { path: { id } },
    }),
  )
}
...
export async function generateCandidateLink(id: string): Promise<CandidateLinkResponse> {
  return handle(
    client.POST('/interviews/{id}/candidate-link', {
      ...LOCALIZED_HEADERS,
      params: { path: { id } },
    }),
  )
...
export async function generateCandidateFeedbackAll(
  ...
  const feedback = postFeedback ?? (await getCandidateFeedback(interviewId, interviewLocale))
...
export async function getInterviewAnswerMedia(
  ...
  return handle(
    client.GET('/interviews/{id}/questions/{questionIndex}/media', {
      ...LOCALIZED_HEADERS,
      params: { path: { id: interviewId, questionIndex } },
    }),
  )
...
export async function getResults(id: string): Promise<InterviewResult> {
  return handle(
    client.GET('/interviews/{id}/results', {
      headers: buildClientBaseHeaders(),
      params: { path: { id } },
    }),
  )
...
export async function sendTakeAnswerProgress(
  ...
  return handle(
    client.POST('/take/{id}/answer/progress', {
      ...LOCALIZED_HEADERS,
      params: { path: { id } },
      body: payload,
    }),
...
export async function updateTemplate(id: string, data: UpdateTemplatePayload): Promise<Template> {
  return handle(
    client.PATCH('/templates/{id}', {
      ...LOCALIZED_HEADERS,
      params: { path: { id } },
      body: data,
    }),
